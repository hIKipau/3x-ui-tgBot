...
4. `/config` проверяет активную подписку, создаёт клиента 3x-ui при первом
   обращении и возвращает одну subscription-ссылку для импорта и автообновления
   всех серверов в V2RayTun. Новый клиент получает
   ...
   последующих запросах старый email синхронизируется с текущим username.
...
- `subscriptions` — тариф, статус, срок, лимит и связь с 3x-ui email;
